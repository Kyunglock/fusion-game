 ...
 *   voteContinue(한 바퀴 더 vs 라이어 지목) → [voteLiar(지목 투표)] →
 *   [liarGuess(라이어가 진짜 제시어 맞히기)] → wordSetup(결과 공개, 다음 라운드 대기)
 *
...
const manager = createRoomManager({
  ...
  extraRoomFields: {
    ...
    accusedId:        null, // voteLiar 결과로 지목된 사람 (liarGuess 단계에서 공개)
  ...
  extraStateFields: (room) => {
    ...
    return {
      ...
      votesIn:      room.state === 'voteContinue' ? Object.keys(room.continueVotes).length
                  : room.state === 'voteLiar'      ? Object.keys(room.accuseVotes).length
                  : 0,
      votesNeeded:  participants.length,
      accusedId:    (room.state === 'liarGuess') ? room.accusedId : null,
